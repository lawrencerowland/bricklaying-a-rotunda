...
(async()=>{
 ...
 try{
  ...
  await page.getByRole('button',{name:'Inspect current tick →',exact:true}).click();assert(await page.locator('#panel-build').isVisible());assert(await page.locator('#settings').isVisible());
  ...
  await page.getByRole('button',{name:'Play scene',exact:true}).click();await page.waitForFunction(()=>document.querySelectorAll('.site-load').length===2&&document.getElementById('site-position').value==='0');await page.getByRole('button',{name:'Pause scene',exact:true}).click();
  await page.getByRole('button',{name:'Inspect current tick →',exact:true}).click();assert.equal(await page.locator('#position').inputValue(),'0');assert.equal(await page.locator('#rings .brick.busy').count(),0);assert.match(await page.locator('#state-caption').innerText(),/0\/2 hoists reserved/);assert.equal(await page.locator('.site-load').count(),0);await page.getByRole('button',{name:'Picture the site',exact:true}).click();
  ...
  await page.getByRole('button',{name:'Inspect current tick →',exact:true}).click();await page.getByLabel('Access to ring B').selectOption('true');await page.getByRole('button',{name:'Construct model',exact:true}).click();await ready();await page.getByRole('button',{name:'Picture the site',exact:true}).click();assert(await page.locator('#site-play').isDisabled());assert.match(await page.locator('#site-b').innerText(),/Access closed/);assert.match(await page.locator('#site-caption').innerText(),/no completion route/);
  assert.deepEqual(errors,[]);console.log(JSON.stringify({result:'PASS',base,checks:'deep link; same model/replay; auto-loop; pause/resume; restart; scrub; current tick handoff; one/two hoists; settings/reload; reduced motion; blocked access; mobile width',pageErrors:errors}));
